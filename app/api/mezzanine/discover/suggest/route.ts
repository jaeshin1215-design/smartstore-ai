...
const CAT_CORE_KEYWORDS: Record<string, string> = {
  ...
  // bakery_fb는 4결 세분 — CAT_EXTRA_RULES에서 전용 규칙으로 제어
  bakery_fb:   "휘낭시에 OR 스콘 OR 약과 OR 드립백 OR 블렌딩티",
...
const CAT_EXTRA_RULES: Record<string, string> = {
  bakery_fb: `[F&B 4결 전용 — 필수 적용]

F&B를 4결로 나눠 5개 쿼리를 아래 배분으로 생성할 것:

결 1 베이커리 (지역=서북권 우선):
  품목명: "휘낭시에" / "스콘" / "크루아상" / "소금빵" — 이 중 1개를 따옴표로
  예: site:instagram.com "휘낭시에" "팝업" 연희 OR 연남 OR 은평 -대행

결 2 디저트·구움과자 (지역=서북권 OR 마포):
  품목명: "약과" / "마들렌" / "쿠키" / "다쿠아즈" — 이 중 1개를 따옴표로
  예: site:instagram.com "약과" "팝업" 서울 OR 마포 OR 서대문 -대행

결 3 로스터리·커피 (지역=서북권 OR 성수 허용):
  품목명: "드립백" / "원두" / "콜드브루" — 이 중 1개를 따옴표로
  ※ "카페" 단독 금지 (대형 계정 수확), "마켓" 금지
  예: site:instagram.com "드립백" "팝업" 서울 OR 마포 OR 성수 -카페체인 -대행

결 4 티·음료 (지역=서울):
  품목명: "전통차" / "블렌딩티" / "콤부차" — 이 중 1개를 따옴표로
  예: site:instagram.com "블렌딩티" "팝업" 서울 OR 마포 OR 서대문 -대행

배분: 결1×1 결2×1 결3×2 결4×1 (총 5개)

공통 필수 규칙:
- "베이커리"·"디저트" 상위어 단독 사용 금지 (대형 계정 수확됨)
- "팝업" 두 번째 따옴표로 반드시 포함
- "마켓" 토큰 쿼리에 절대 넣지 말 것
- "스마트스토어" 쿼리에 절대 넣지 말 것
- 빵력장터 브랜드명(봄날엔 등) 쿼리에 넣지 말 것
- 제외어: -대행 1개만 (복수 제외어 쌓기 금지 — 결과 0건)
- 팔로워 기준: 5천~5만 중간 크기 (생과방·버터베이커리급 통과, 73K 이상 불필요)`,
};
