      ...
      try {
        ...
        try {
          const raw = await callClaude(
            ...
            150, { feature: "naver-trend-season" }
          );
        ...
        if (hotKeywords.length > 0 && hotKeywords[0].avg > 0) {
          try {
            ...
            const raw = await callClaude(
              ...
              400, { feature: "naver-trend-comment" }
            );
      ...
      try {
        ...
        try {
          const raw = await callClaude(
            ...
            150, { feature: "naver-trend-related" }
          );
...
JSON만 응답: {"verdict":"good 또는 bad 또는 neutral","action_command":"지금 [액션] — 이유 한 문장","reason":"수치 근거 2문장","tips":["오늘 할 것","이번 주 할 것","타겟 공략 팁"]}`;
        ...
        let rawAi = "";
        try {
          const aiGeminiStream = createGeminiStream(aiPrompt, 300, { feature: "naver-trend-ai" });
