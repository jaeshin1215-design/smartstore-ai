...
이지스토리 회신 진행 중. 아래 3건이 현재 대기다.

- **#A 정산 2차 — 방식·공수 결정 대기**: 집계표 안은 고객이 거절(드릴다운이 매주 쓰는 수정 경로). 피벗 유지로 확정.
  실제 Excel로 검증 완료 — 피벗 소스를 **엑셀 표(ListObject)** 로 두면 행 추가 자동 확장 + "(비어 있음)" 소멸,
  손익표는 **GETPIVOTDATA 아닌 SUMIFS**로 정제후 직접 참조(원본 수정 즉시 반영, 새로고침 불필요).
  GETPIVOTDATA는 이름 바뀐 AI열("합계 : 원가")에서 `#REF!` — 쓰지 말 것.
  **Jae 결정 대기**: 완전 자동(템플릿 zip 치환, `fflate` 사용, 5~7일) vs 반자동(템플릿 1회 제작, 1.5~2일).
  라이브러리 교체 불필요 — 기존 정산·발주서·송장 기능 영향 0.
- **#B 송장 미기재 분리 시트 (0.5일)**: 「취소」가 아니라 송장번호 공란 행 분리. 자동 삭제·제외 금지.
  현재 로직은 공란 행을 정제후·집계에 **전부 포함**(dedup만 격리 — settlement-process.ts:171).
  고객 현행은 손익표에서 **제외**(이번 주 2행 · 상품매출 30,322 · 총합계의 1.25%). 제외 + 파일 내 잔존 + 안내 한 줄 권고.
- **#C 개인별 토큰 체계 (과제)**: EXTENSION_TOKEN 단일 공유값. 기기 식별·발급이력·동시제한 전부 없고 스키마에 토큰 열도 없음.
  교체 시 전원 동시 중단. 고객이 토큰을 메신저에 붙여넣는 것 확인됨(2026-09-11).

### 이전 백로그 (2026-07-24 기준 · 상태 미확인)
