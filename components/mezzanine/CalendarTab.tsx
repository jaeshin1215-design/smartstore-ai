...
export default function CalendarTab({ filter, onNavigate, onFilterChange }: CalendarTabProps) {
  ...
  function getBrandsForMonth(month: number): { brand: CalBrand; role: "target" | "fb" }[] {
    // DB month 컬럼 기준으로 해당 달에 배정된 브랜드만 필터링
    const monthBrands = brands
      .filter(b => b.month === month && b.status === "MANUAL_VERIFIED")
    ...
    // F&B 헤드라이너 — bakery_fb
    for (const b of monthBrands.filter(b => b.category === "bakery_fb")) {
      if (!used.has(b.id)) { result.push({ brand: b, role: "fb" }); used.add(b.id); }
    }

    // TARGET PARTNERS — bakery_fb 제외 나머지
    for (const b of monthBrands.filter(b => b.category !== "bakery_fb")) {
